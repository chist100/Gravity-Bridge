...
func TestBatches(t *testing.T) {
	...
	var (
		...
		token, e4               = types.NewInternalERC20Token(math.NewInt(99999), myTokenContractAddr.GetAddress().Hex())
	...
	for i, v := range []uint64{2, 3, 2, 1} {
		amountToken, err := types.NewInternalERC20Token(math.NewInt(int64(i+100)), myTokenContractAddr.GetAddress().Hex())
		require.NoError(t, err)
		amount := amountToken.GravityCoin()
		feeToken, err := types.NewInternalERC20Token(math.NewIntFromUint64(v), myTokenContractAddr.GetAddress().Hex())
		require.NoError(t, err)
	...
	oneFee, err := types.NewInternalERC20Token(math.NewInt(1), myTokenContractAddr.GetAddress().Hex())
	require.NoError(t, err)
	oneHundredTok, err := types.NewInternalERC20Token(math.NewInt(100), myTokenContractAddr.GetAddress().Hex())
	require.NoError(t, err)
	twoFee, err := types.NewInternalERC20Token(math.NewInt(2), myTokenContractAddr.GetAddress().Hex())
	require.NoError(t, err)
	oneHundredThreeTok, err := types.NewInternalERC20Token(math.NewInt(103), myTokenContractAddr.GetAddress().Hex())
	require.NoError(t, err)
	...
	gotUnbatchedTx = input.GravityKeeper.GetUnbatchedTransactionsByContract(ctx, *myTokenContractAddr)
	threeFee, err := types.NewInternalERC20Token(math.NewInt(3), myTokenContractAddr.GetAddress().Hex())
	require.NoError(t, err)
	oneHundredOneTok, err := types.NewInternalERC20Token(math.NewInt(101), myTokenContractAddr.GetAddress().Hex())
	require.NoError(t, err)
	oneHundredTwoTok, err := types.NewInternalERC20Token(math.NewInt(102), myTokenContractAddr.GetAddress().Hex())
	require.NoError(t, err)
	expUnbatchedTx = []*types.InternalOutgoingTransferTx{
		{
			Id:          2,
...
func TestBatchesFullCoins(t *testing.T) {
	...
	expFirstBatch := &types.OutgoingTxBatch{
		...
		Transactions: []types.OutgoingTransferTx{
			{
				Id:          2,
				Erc20Fee:    types.NewSDKIntERC20Token(oneEth.Mul(math.NewIntFromUint64(300)), myTokenContractAddr),
				Sender:      mySender.String(),
				DestAddress: myReceiver,
				Erc20Token:  types.NewSDKIntERC20Token(oneEth.Mul(math.NewIntFromUint64(300)), myTokenContractAddr),
			},
			{
				Id:          3,
				Erc20Fee:    types.NewSDKIntERC20Token(oneEth.Mul(math.NewIntFromUint64(25)), myTokenContractAddr),
				Sender:      mySender.String(),
				DestAddress: myReceiver,
				Erc20Token:  types.NewSDKIntERC20Token(oneEth.Mul(math.NewIntFromUint64(25)), myTokenContractAddr),
			},
		},
	...
	twentyTok, err := types.NewInternalERC20Token(oneEth.Mul(math.NewIntFromUint64(20)), myTokenContractAddr)
	require.NoError(t, err)
	tenTok, err := types.NewInternalERC20Token(oneEth.Mul(math.NewIntFromUint64(10)), myTokenContractAddr)
	require.NoError(t, err)
	expUnbatchedTx := []*types.InternalOutgoingTransferTx{
		{
			Id:          1,
	...
	expSecondBatch := &types.OutgoingTxBatch{
		...
		Transactions: []types.OutgoingTransferTx{
			{
				Id:          5,
				Erc20Fee:    types.NewSDKIntERC20Token(oneEth.Mul(math.NewIntFromUint64(200)), myTokenContractAddr),
				Sender:      mySender.String(),
				DestAddress: myReceiver,
				Erc20Token:  types.NewSDKIntERC20Token(oneEth.Mul(math.NewIntFromUint64(200)), myTokenContractAddr),
			},
			{
				Id:          6,
				Erc20Fee:    types.NewSDKIntERC20Token(oneEth.Mul(math.NewIntFromUint64(150)), myTokenContractAddr),
				Sender:      mySender.String(),
				DestAddress: myReceiver,
				Erc20Token:  types.NewSDKIntERC20Token(oneEth.Mul(math.NewIntFromUint64(150)), myTokenContractAddr),
			},
		},
		TokenContract:      myTokenContractAddr,
		CosmosBlockCreated: 1234567,
	...
	// check that txs from first batch have been freed
	gotUnbatchedTx = input.GravityKeeper.GetUnbatchedTransactionsByContract(ctx, *tokenContract)
	threeHundredTok, err := types.NewInternalERC20Token(oneEth.Mul(math.NewIntFromUint64(300)), myTokenContractAddr)
	require.NoError(t, err)
	twentyFiveTok, err := types.NewInternalERC20Token(oneEth.Mul(math.NewIntFromUint64(25)), myTokenContractAddr)
	require.NoError(t, err)
	expUnbatchedTx = []*types.InternalOutgoingTransferTx{
		{
			Id:          2,
...
func TestPoolTxRefund(t *testing.T) {
	...
	var (
		...
		myReceiver          = "0xd041c41EA1bf0F006ADBb6d2c9ef9D425dE5eaD7"
		myTokenContractAddr = "0x429881672B9AE42b8EbA0E26cD9C73711b891Ca5" // Pickle
		token, e3           = types.NewInternalERC20Token(math.NewInt(414), myTokenContractAddr)
		allVouchers         = sdk.NewCoins(token.GravityCoin())
		denomToken, e4      = types.NewInternalERC20Token(math.NewInt(1), myTokenContractAddr)
	...
	require.Equal(t, math.NewInt(104), balances.AmountOf(myDenom))
}
...
func TestEthereumBlacklistBatches(t *testing.T) {
	...
	var (
		...
		token, e5               = types.NewInternalERC20Token(math.NewInt(99999), myTokenContractAddr.GetAddress().Hex())
		allVouchers             = sdk.NewCoins(token.GravityCoin())
	)
	require.NoError(t, e1)
	require.NoError(t, e2)
	...
	assert.True(t, currentFees.TotalFees.Equal(math.NewInt(8)))
...
func TestGetFees(t *testing.T) {
	...
	txs := []types.OutgoingTransferTx{
		{Erc20Fee: types.ERC20Token{Amount: math.NewInt(1)}},
		{Erc20Fee: types.ERC20Token{Amount: math.NewInt(2)}},
		{Erc20Fee: types.ERC20Token{Amount: math.NewInt(3)}},
	}
...
func TestBatchConfirms(t *testing.T) {
	...
	var (
		...
		myReceiver, e2          = types.NewEthAddress("0xd041c41EA1bf0F006ADBb6d2c9ef9D425dE5eaD7")
		myTokenContractAddr, e3 = types.NewEthAddress("0x429881672B9AE42b8EbA0E26cD9C73711b891Ca5") // Pickle
		token, e4               = types.NewInternalERC20Token(math.NewInt(1000000), myTokenContractAddr.GetAddress().Hex())
		allVouchers             = sdk.NewCoins(token.GravityCoin())
	)
	require.NoError(t, e1)
	require.NoError(t, e2)
	...
	for i := 1; i < 200; i++ {
		...
		require.NoError(t, err)
		amount := amountToken.GravityCoin()
		feeToken, err := types.NewInternalERC20Token(math.NewIntFromUint64(uint64(i+10)), myTokenContractAddr.GetAddress().Hex())
		require.NoError(t, err)
		fee := feeToken.GravityCoin()
