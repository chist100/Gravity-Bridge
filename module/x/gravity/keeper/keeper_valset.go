...
func (k Keeper) GetCurrentValset(ctx sdk.Context) (types.Valset, error) {
	...
	for _, validator := range validators {
		...
		if err := sdk.VerifyAddressFormat([]byte(val)); err != nil {
			return types.Valset{}, errorsmod.Wrap(err, types.ErrInvalidValAddress.Error())
		}

		power, _ := k.StakingKeeper.GetLastValidatorPower(ctx, []byte(val))
		p := math.NewInt(power)

		if ethAddr, found := k.GetEthAddressByValidator(ctx, []byte(val)); found {
