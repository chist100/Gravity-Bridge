...
func TestAddToOutgoingPool(t *testing.T) {
	...
	allVouchersToken, err := types.NewInternalERC20Token(math.NewInt(99999), myTokenContractAddr)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	threeTok, err := types.NewInternalERC20Token(math.NewInt(3), myTokenContractAddr)
	require.NoError(t, err)
	twoTok, err := types.NewInternalERC20Token(math.NewInt(2), myTokenContractAddr)
	require.NoError(t, err)
	oneTok, err := types.NewInternalERC20Token(math.NewInt(1), myTokenContractAddr)
	require.NoError(t, err)
	oneHundredTok, err := types.NewInternalERC20Token(math.NewInt(100), myTokenContractAddr)
	require.NoError(t, err)
	oneHundredOneTok, err := types.NewInternalERC20Token(math.NewInt(101), myTokenContractAddr)
	require.NoError(t, err)
	oneHundredTwoTok, err := types.NewInternalERC20Token(math.NewInt(102), myTokenContractAddr)
	require.NoError(t, err)
	oneHundredThreeTok, err := types.NewInternalERC20Token(math.NewInt(103), myTokenContractAddr)
	require.NoError(t, err)
...
func TestAddToOutgoingPoolEdgeCases(t *testing.T) {
	...
	receiver, err := types.NewEthAddress(myReceiver)
	require.NoError(t, err)
	amountToken, err := types.NewInternalERC20Token(math.NewInt(int64(100)), myTokenContractAddr)
	require.NoError(t, err)
	amount := amountToken.GravityCoin()
	feeToken, err := types.NewInternalERC20Token(math.NewInt(2), myTokenContractAddr)
	require.NoError(t, err)
	fee := feeToken.GravityCoin()
	...
	badAmountToken, err := types.NewInternalERC20Token(math.NewInt(999999), myTokenContractAddr)
	require.NoError(t, err)
	...
	badFeeToken, err := types.NewInternalERC20Token(math.NewInt(999999), myTokenContractAddr)
	require.NoError(t, err)
	...
	badFeeToken, err = types.NewInternalERC20Token(math.NewInt(99999-99), myTokenContractAddr)
	require.NoError(t, err)
	badFee = badFeeToken.GravityCoin()
	r, err = input.GravityKeeper.AddToOutgoingPool(ctx, mySender, *receiver, amount, badFee)
	require.Error(t, err)
	...
	badFeeToken, err = types.NewInternalERC20Token(math.NewInt(100), badFeeContractAddr)
	require.NoError(t, err)
	badFee = badFeeToken.GravityCoin()
	r, err = input.GravityKeeper.AddToOutgoingPool(ctx, mySender, *receiver, amount, badFee)
	require.Error(t, err)
...
func TestTotalBatchFeeInPool(t *testing.T) {
	...
	// mint some voucher first
	allVouchersToken, err = types.NewInternalERC20Token(math.NewIntFromUint64(uint64(18446744073709551615)), myToken2ContractAddr)
	require.NoError(t, err)
	allVouchers = sdk.Coins{allVouchersToken.GravityCoin()}
	err = input.BankKeeper.MintCoins(ctx, types.ModuleName, allVouchers)
	require.NoError(t, err)
	...
	// create outgoing pool
	for i := 0; i < 110; i++ {
		amountToken, err := types.NewInternalERC20Token(math.NewInt(int64(i+100)), myToken2ContractAddr)
		require.NoError(t, err)
		amount := amountToken.GravityCoin()
		feeToken, err := types.NewInternalERC20Token(math.NewIntFromUint64(5), myToken2ContractAddr)
		require.NoError(t, err)
		fee := feeToken.GravityCoin()
...
func TestGetBatchFeeByTokenType(t *testing.T) {
	...
	allVouchersToken1, err := types.NewInternalERC20Token(math.NewInt(99999), myTokenContractAddr1)
	require.NoError(t, err)
	allVouchers1 := sdk.Coins{allVouchersToken1.GravityCoin()}
	allVouchersToken2, err := types.NewInternalERC20Token(math.NewInt(99999), myTokenContractAddr2)
	require.NoError(t, err)
	allVouchers2 := sdk.Coins{allVouchersToken2.GravityCoin()}
	allVouchersToken3, err := types.NewInternalERC20Token(math.NewInt(99999), myTokenContractAddr3)
	require.NoError(t, err)
	...
	// create outgoing pool
	for i := 0; i < 110; i++ {
		amountToken1, err := types.NewInternalERC20Token(math.NewInt(int64(i+100)), myTokenContractAddr1)
		require.NoError(t, err)
		amount1 := amountToken1.GravityCoin()
		feeAmt1 := int64(i + 1) // fees can't be 0
		feeToken1, err := types.NewInternalERC20Token(math.NewInt(feeAmt1), myTokenContractAddr1)
		require.NoError(t, err)
		fee1 := feeToken1.GravityCoin()
		amountToken2, err := types.NewInternalERC20Token(math.NewInt(int64(i+100)), myTokenContractAddr2)
		require.NoError(t, err)
		amount2 := amountToken2.GravityCoin()
		feeAmt2 := int64(2*i + 1) // fees can't be 0
		feeToken2, err := types.NewInternalERC20Token(math.NewInt(feeAmt2), myTokenContractAddr2)
		require.NoError(t, err)
		fee2 := feeToken2.GravityCoin()
		amountToken3, err := types.NewInternalERC20Token(math.NewInt(int64(i+100)), myTokenContractAddr3)
		require.NoError(t, err)
		amount3 := amountToken3.GravityCoin()
		feeAmt3 := int64(3*i + 1) // fees can't be 0
		feeToken3, err := types.NewInternalERC20Token(math.NewInt(feeAmt3), myTokenContractAddr3)
		require.NoError(t, err)
...
func TestRemoveFromOutgoingPoolAndRefund(t *testing.T) {
	...
	allVouchersToken, err := types.NewInternalERC20Token(math.NewIntFromUint64(originalBal), myTokenContractAddr)
	require.NoError(t, err)
	allVouchers := sdk.Coins{allVouchersToken.GravityCoin()}
	err = input.BankKeeper.MintCoins(ctx, types.ModuleName, allVouchers)
	require.NoError(t, err)
	...
	for i, v := range fees {
		amountToken, err := types.NewInternalERC20Token(math.NewIntFromUint64(amounts[i]), myTokenContractAddr)
		require.NoError(t, err)
		amount := amountToken.GravityCoin()
		feeToken, err := types.NewInternalERC20Token(math.NewIntFromUint64(v), myTokenContractAddr)
		require.NoError(t, err)
...
func TestRemoveFromOutgoingPoolAndRefundCosmosOriginated(t *testing.T) {
	...
	for i, v := range fees {
		...
		fee := sdk.NewCoin(myTokenDenom, math.NewIntFromUint64(v))
...
func TestRefundInconsistentTx(t *testing.T) {
	...
	amountToken, err := types.NewInternalERC20Token(math.NewInt(100), myTokenContractAddr.GetAddress().Hex())
	require.NoError(t, err)
	badTokenContractAddr, err := types.NewEthAddress("0x429881672b9AE42b8eBA0e26cd9c73711b891ca6") // different last char
	require.NoError(t, err)
	badFeeToken, err := types.NewInternalERC20Token(math.NewInt(2), badTokenContractAddr.GetAddress().Hex())
	require.NoError(t, err)
...
func TestRefundTwice(t *testing.T) {
	...
	amountToken, err := types.NewInternalERC20Token(math.NewInt(100), myTokenContractAddr)
	require.NoError(t, err)
	feeToken, err := types.NewInternalERC20Token(math.NewInt(2), myTokenContractAddr)
	require.NoError(t, err)
	origBalances := input.BankKeeper.GetAllBalances(ctx, mySender)
...
func TestGetUnbatchedTransactions(t *testing.T) {
	...
	for i, v := range fees {
		amountToken1, err := types.NewInternalERC20Token(math.NewIntFromUint64(amounts[i]), myTokenContractAddr1)
		require.NoError(t, err)
		amount1 := amountToken1.GravityCoin()
		feeToken1, err := types.NewInternalERC20Token(math.NewIntFromUint64(v), myTokenContractAddr1)
		require.NoError(t, err)
		fee1 := feeToken1.GravityCoin()
		...
		amountToken2, err := types.NewInternalERC20Token(math.NewIntFromUint64(amounts[i]), myTokenContractAddr2)
		require.NoError(t, err)
		amount2 := amountToken2.GravityCoin()
		feeToken2, err := types.NewInternalERC20Token(math.NewIntFromUint64(v), myTokenContractAddr2)
		require.NoError(t, err)
		fee2 := feeToken2.GravityCoin()
	...
	token1Fee, err := types.NewInternalERC20Token(math.NewIntFromUint64(fees[0]), myTokenContractAddr1)
	require.NoError(t, err)
	token1Amount, err := types.NewInternalERC20Token(math.NewIntFromUint64(amounts[0]), myTokenContractAddr1)
	require.NoError(t, err)
	token1Id := ids1[0]
	...
	token2Fee, err := types.NewInternalERC20Token(math.NewIntFromUint64(fees[3]), myTokenContractAddr2)
	require.NoError(t, err)
	token2Amount, err := types.NewInternalERC20Token(math.NewIntFromUint64(amounts[3]), myTokenContractAddr2)
	require.NoError(t, err)
...
func TestIterateUnbatchedTransactions(t *testing.T) {
	...
	require.NoError(t, err)
	// mint some vouchers first
	token1, err := types.NewInternalERC20Token(math.NewInt(99999), myTokenContractAddr1)
	require.NoError(t, err)
	allVouchers1 := sdk.Coins{token1.GravityCoin()}
	err = input.BankKeeper.MintCoins(ctx, types.ModuleName, allVouchers1)
	require.NoError(t, err)

	token2, err := types.NewInternalERC20Token(math.NewInt(99999), myTokenContractAddr2)
	require.NoError(t, err)
	...
	idToTxMap := make(map[uint64]*types.OutgoingTransferTx)
	for i, v := range fees {
		amount1, err := types.NewInternalERC20Token(math.NewIntFromUint64(amounts[i]), myTokenContractAddr1)
		require.NoError(t, err)
		fee1, err := types.NewInternalERC20Token(math.NewIntFromUint64(v), myTokenContractAddr1)
		require.NoError(t, err)
		...
		amount2, err := types.NewInternalERC20Token(math.NewIntFromUint64(amounts[i]), myTokenContractAddr2)
		require.NoError(t, err)
		fee2, err := types.NewInternalERC20Token(math.NewIntFromUint64(v), myTokenContractAddr2)
		require.NoError(t, err)
