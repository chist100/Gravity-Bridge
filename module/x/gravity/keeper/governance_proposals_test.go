...
func TestAirdropProposal(t *testing.T) {
	...
	extremelyLargeAmount := math.NewInt(1000000000000).Mul(math.NewInt(1000000000000))
	...
	feePool, _ := gk.DistKeeper.FeePool.Get(ctx)
	...
	feePoolEq, _ := input.DistKeeper.FeePool.Get(ctx)
	assert.Equal(t, feePoolEq, feePool)
