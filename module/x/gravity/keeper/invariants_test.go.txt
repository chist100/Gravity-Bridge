...
func TestModuleBalanceUnbatchedTxs(t *testing.T) {
	...
	oneVoucher, err := types.NewInternalERC20Token(math.NewInt(1), myTokenContractAddr)
	require.NoError(t, err)
...
func TestModuleBalanceBatchedTxs(t *testing.T) {
	...
	var err error
	tokens[0], err = types.NewInternalERC20Token(math.NewInt(150000000000000), myTokenContractAddr1.GetAddress().Hex())
	require.NoError(t, err)
	tokens[1], err = types.NewInternalERC20Token(math.NewInt(150000000000000), myTokenContractAddr2.GetAddress().Hex())
	require.NoError(t, err)
	voucher1, err := types.NewInternalERC20Token(math.NewInt(1), myTokenContractAddr1.GetAddress().Hex())
	require.NoError(t, err)
	voucher2, err := types.NewInternalERC20Token(math.NewInt(1), myTokenContractAddr2.GetAddress().Hex())
	require.NoError(t, err)
	...
	for _, tok := range tokens {
		...
		for i, v := range []uint64{2, 3, 2, 1, 2, 4, 5, 1} {
			amountToken, err := types.NewInternalERC20Token(math.NewInt(int64(i+100)), tok.Contract.GetAddress().Hex())
			require.NoError(t, err)
			amount := amountToken.GravityCoin()
			feeToken, err := types.NewInternalERC20Token(math.NewIntFromUint64(v), tok.Contract.GetAddress().Hex())
			require.NoError(t, err)
			fee := feeToken.GravityCoin()
