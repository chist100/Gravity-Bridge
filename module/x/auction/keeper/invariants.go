...
func ExpectedAuctionModuleBalances(ctx sdk.Context, k Keeper) sdk.Coins {
	param, err := k.MintKeeper.Params.Get(ctx)
	if err != nil {
		panic(err)
	}
	bidToken := param.MintDenom
	var highestBids = math.ZeroInt()
	...
	k.IterateAuctions(ctx, func(_ []byte, auction types.Auction) (stop bool) {
		...
		if auction.HighestBid != nil {
			highestBids = highestBids.Add(math.NewIntFromUint64(auction.HighestBid.BidAmount))
		}
