...
func (m msgServer) Bid(goCtx context.Context, msg *types.MsgBid) (res *types.MsgBidResponse, err error) {
	...
	paras, err := m.MintKeeper.Params.Get(ctx)
	if err != nil {
		return nil, err
	}
	bidToken := paras.MintDenom
	minBidFee := math.NewIntFromUint64(params.MinBidFee)
	...
	bidAmount := math.NewIntFromUint64(msg.Amount)

	// Check the supplied fee meets the minimum
	feeInt := math.NewIntFromUint64(msg.BidFee)
	...
	if highestBid != nil {
		if bidAmount.LT(math.NewIntFromUint64(highestBid.BidAmount)) {
	...
	if highestBid != nil {
		oldBid = sdk.NewCoin(bidToken, math.NewIntFromUint64(highestBid.BidAmount))
	...
	newBid = sdk.NewCoin(config.NativeTokenDenom, math.NewIntFromUint64(updatedBid.BidAmount))

	// Emit an event to mark a new highest bidder
	ctx.EventManager().EmitEvent(types.NewEventNewHighestBidder(msg.AuctionId, math.NewIntFromUint64(msg.Amount), oldBidder))
